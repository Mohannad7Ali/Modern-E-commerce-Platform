...
export default function generatePDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.on('error', (err: Error) => reject(new Error(`PDF generation failed: ${err.message}`)));

    try {
      doc.fontSize(16).font('Helvetica-Bold').text('Report', { align: 'center' }).moveDown(1);
      ...
      if ('sales' in data && 'userRetention' in data) {
        ...
        // Sales Section
        doc.fontSize(12).font('Helvetica').text('Sales Report', { underline: true }).moveDown(0.5);
        doc
          .fontSize(10)
        ...
        doc.fontSize(12).text('User Retention Report', { underline: true }).moveDown(0.5);
        doc
          .fontSize(10)
        ...
        // UserRetentionReport
        const { totalUsers, retentionRate, repeatPurchaseRate, lifetimeValue, topUsers } = data as any;
        doc.fontSize(12).font('Helvetica').text('User Retention Report', { underline: true }).moveDown(0.5);
        doc
          .fontSize(10)
        ...
        // SalesReport
        const { totalRevenue, totalOrders, totalSales, averageOrderValue, byCategory, topProducts } = data as any;
        doc.fontSize(12).font('Helvetica').text('Sales Report', { underline: true }).moveDown(0.5);
        doc
          .fontSize(10)
        ...
        doc.fontSize(12).font('Helvetica').text('Analytics Overview', { underline: true }).moveDown(0.5);
        doc
          .fontSize(10)
        ...
        doc
          .fontSize(10)
          .text(`Revenue Change: ${overview.changes.revenue ? overview.changes.revenue.toFixed(2) + '%' : 'N/A'}`)
          .text(`Orders Change: ${overview.changes.orders ? overview.changes.orders.toFixed(2) + '%' : 'N/A'}`)
          .text(`Sales Change: ${overview.changes.sales ? overview.changes.sales.toFixed(2) + '%' : 'N/A'}`)
          .text(`Users Change: ${overview.changes.users ? overview.changes.users.toFixed(2) + '%' : 'N/A'}`)
          .text(
            `AOV Change: ${
              overview.changes.averageOrderValue ? overview.changes.averageOrderValue.toFixed(2) + '%' : 'N/A'
            }`
          )
          .moveDown(0.5);
        doc.fontSize(12).text('Monthly Trends', { underline: true }).moveDown(0.5);
        ...
        doc.fontSize(12).text('Product Performance', { underline: true }).moveDown(0.5);
        ...
        doc.fontSize(12).text('User Analytics', { underline: true }).moveDown(0.5);
        doc
          .fontSize(10)
          ...
          .text(`Users Change: ${users.changes?.users ? users.changes.users.toFixed(2) + '%' : 'N/A'}`)
          .moveDown(1);
        doc.fontSize(12).text('Top Users', { underline: true }).moveDown(0.5);
        ...
        doc.fontSize(12).text('Interaction Trends', { underline: true }).moveDown(0.5);
