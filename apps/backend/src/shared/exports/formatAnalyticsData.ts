import { AllAnalytics, AnalyticsOverview, ExportableData, UserAnalytics } from '@/modules/analytics/analytics.types';
...
export default function formatAnalyticsData(data: ExportableData): any[] {
  ...
  } else if ('totalUsers' in data && 'topUsers' in data && 'interactionTrends' in data) {
    // UserAnalytics
    const { topUsers, interactionTrends, changes, ...rest } = data as UserAnalytics;
