...
export default async function generateXLSX(
  ...
  if ('sales' in data && 'userRetention' in data) {
    ...
    sales.byCategory.forEach(cat => {
      salesSheet.addRow([cat.categoryId, cat.categoryName, cat.revenue, cat.sales]);
    });
    salesSheet.addRow([]); // Spacer
    salesSheet.addRow(['Top Products']);
    salesSheet.addRow(['Product ID', 'Product Name', 'Quantity', 'Revenue ($)']);
    sales.topProducts.forEach(prod => {
      salesSheet.addRow([prod.productId, prod.productName, prod.quantity, prod.revenue]);
    });
    ...
    retentionSheet.addRow(['Rank', 'User ID', 'Name', 'Email', 'Order Count', 'Total Spent ($)']);
    userRetention.topUsers.forEach((user: any, index: number) => {
      retentionSheet.addRow([index + 1, user.userId, user.name, user.email, user.orderCount, user.totalSpent]);
    });
    ...
    sheet.addRow(['Rank', 'User ID', 'Name', 'Email', 'Order Count', 'Total Spent ($)']);
    topUsers.forEach((user: any, index: number) => {
      sheet.addRow([index + 1, user.userId, user.name, user.email, user.orderCount, user.totalSpent]);
    });
    ...
    topProducts.forEach((prod: any) => {
      sheet.addRow([prod.productId, prod.productName, prod.quantity, prod.revenue]);
    });
    ...
    usersSheet.addRow(['Rank', 'Name', 'Email', 'Order Count', 'Total Spent ($)', 'Engagement Score']);
    users.topUsers.forEach((user: any, index: number) => {
      usersSheet.addRow([index + 1, user.name, user.email, user.orderCount, user.totalSpent, user.engagementScore]);
    });
