class ApiFeatures {
  ...
  constructor(queryString: Record<string, any>) {
    this.queryOptions = {}; // الكائن النهائي الذي سيُرسل لـ Prisma
    this.queryString = queryString || {}; // البيانات القادمة من req.query
  }
  /**
   * (URL with query string): https://api.myshop.com/products?category=electronics&featured=true&sort=price:desc
   * when this querySting arrive express server , express use qs and convert it to js object and put it inside request req.query
    const queryString = {
  ...
  */
  filter(): this {
    ...
    // intialize the filters object that will become the prisma 'where' clause.
    ...
    if (this.queryString.bestselling) {
      ...
      filters.bestSeller = this.queryString.bestselling.toLowerCase() === 'true';
    ...
    if (this.queryString.promotional) {
      filters.promotional = this.queryString.promotional.toLowerCase() === 'true';
  ...
  limitFields(): this {
    if (this.queryString.fields) {
      const fields = this.queryString.fields.split(',').reduce((accept: any, field: string) => {
        accept[field] = true;
        return accept;
      }, {});
      //select : {field:true , x:true ,,,}
