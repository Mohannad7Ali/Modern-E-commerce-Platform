...
import generateCSV from '@/shared/exports/generateCsv';
import generatePDF from '@/shared/exports/generatePdf';
import generateXLSX from '@/shared/exports/generateXlsx';
export class AnalyticsController {
  ...
  exportAnalytics = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!type || !validTypes.includes(type as string)) {
      throw new AppError(400, 'Invalid type. Use: overview, products, users, or all');
    }

    const validPeriods = ['last7days', 'lastMonth', 'lastYear', 'allTime', 'custom'];
    if (!timePeriod || !validPeriods.includes(timePeriod as string)) {
      throw new AppError(400, 'Invalid or missing timePeriod. Use: last7days, lastMonth, lastYear, allTime, or custom');
    }
    ...
    if (startDate && endDate) {
      ...
      if (isNaN(customStartDate.getTime()) || isNaN(customEndDate.getTime())) {
        throw new AppError(400, 'Invalid startDate or endDate format. Use YYYY-MM-DD.');
      }
      ...
      throw new AppError(400, 'Both startDate and endDate must be provided for a custom range.');
    }
