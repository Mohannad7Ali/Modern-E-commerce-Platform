import { Context } from '@/modules/product/graphql/productResolver';
import searchModel from '@/shared/utils/searchModel';

export const searchDashboardResolver = {
  Query: {
    searchDashboard: async (_: any, { params }: { params: { searchQuery: string } }, { prisma }: Context) => {
      const { searchQuery } = params;

      const transactions = await searchModel('transaction', [{ name: 'status', isString: false }], searchQuery, prisma);
