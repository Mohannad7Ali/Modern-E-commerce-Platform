  ...
  Query: {
    revenueAnalytics: async (_: any, { params }: any, { prisma }: Context) => {
      ...
      const previousOrders = fetchPrevious
        ? await fetchData(prisma, 'order', 'orderDate', previousStartDate, previousEndDate, yearStart, yearEnd)
        : [];
      ...
      const ordersForTrends = await fetchData(prisma, 'order', 'createdAt', yearStart, yearEnd);
      const orderItemsForTrends = await fetchData(prisma, 'orderItem', 'createdAt', yearStart, yearEnd, undefined);
      const usersForTrends = await fetchData(prisma, 'user', 'createdAt', yearStart, yearEnd);
