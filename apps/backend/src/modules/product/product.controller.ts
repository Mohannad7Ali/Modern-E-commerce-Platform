...
export class ProductController {
  ...
  getProductById = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const productId = CheckParamsType(req.params.id);
  ...
  getProductBySlug = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const productSlug = CheckParamsType(req.params.id);
    ...
    const processedVariants = parsedVariants.length
      ? await Promise.all(
          parsedVariants.map(async (variant: any, index: number) => {
            ...
            try {
              imageIndexes = variant.imageIndexes ? JSON.parse(variant.imageIndexes) : [];
            ...
            if (typeof bodyImages === 'string') {
              try {
                ...
                throw new AppError(400, `Invalid images format at variant index ${index}`);
              }
            }
            if (!Array.isArray(bodyImages) || bodyImages.some((img: any) => img && typeof img !== 'string')) {
              throw new AppError(400, `Images at variant index ${index} must be an array of strings or empty`);
            }

            // Combine uploaded images with body images
            imageUrls = [...imageUrls, ...bodyImages.filter((img: string) => img)];
            ...
            try {
              ...
              console.log(error);
              throw new AppError(400, `Invalid attributes format at index ${index}`);
            }
