...
export default class OrderService {
  ...
  async getAllOrders() {
    ...
    if (!orders || orders.length === 0) {
      return new AppError(404, 'No orders found');
    }
    return orders;
  }
  ...
  async getUserOrders(userId: string) {
    ...
    if (!orders || orders.length === 0) {
      return new AppError(404, 'No orders found for this user');
    }
    return orders;
  }
  async getOrderDetails(orderId: string, userId: string) {
    ...
    if (order.userId !== userId) {
      return new AppError(403, 'You are not authorized to view this order');
    }
    return order;
  }
