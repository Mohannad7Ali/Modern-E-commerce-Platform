...
import { CART_EVENT } from '@/generated/prisma-client/client';
...
export class CartService {
  ...
  async logCartEvent(cartId: string, eventType: CART_EVENT, userId?: string): Promise<void> {
  ...
  async getAbandonedCartMetrics(
    ...
    for (const cartId in cartEventsByCartId) {
      ...
      const hasCheckoutCompleted = events.some((e: any) => e.eventType === 'CHECKOUT_COMPLETED');
  ...
  async addToCart(variantId: string, quantity: number, userId?: string, sessionId?: string) {
