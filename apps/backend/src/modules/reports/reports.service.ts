...
import { DateRangeQuery, SalesReport, UserRetentionReport } from './reports.types';
...
export class ReportsService {
  ...
  async generateUserRetentionReport(query: DateRangeQuery): Promise<UserRetentionReport> {
    const { timePeriod, year, startDate, endDate } = query;
    ...
    const { currentStartDate, currentEndDate, previousStartDate, previousEndDate, yearStart, yearEnd } =
  ...
  async logReport(data: { type: string; format: string; userId?: string; parameters: DateRangeQuery }) {
