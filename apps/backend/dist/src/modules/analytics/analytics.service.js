...
class AnalyticsService {
    ...
    async getMonthlyTrends(startDate, endDate) {
        ...
        while (current <= endDate) {
            ...
            const filteredOrders = allOrders.filter(o => o.createdAt.getFullYear() === year && o.createdAt.getMonth() === month);
    ...
    async getAnalyticsOverview(query) {
        ...
        const prevOrders = await prisma_1.prisma.order.findMany({
        ...
        return {
            ...
            changes: {
                ...
                averageOrderValue: prevAverageOrderValue > 0 ? ((averageOrderValue - prevAverageOrderValue) / prevAverageOrderValue) * 100 : null
            },
            monthlyTrends
        };
    }
    ...
    async getInteractionTrends(startDate, endDate) {
        ...
        const allInteractions = await prisma_1.prisma.interaction.findMany({
            where: {
                createdAt: { gte: startDate, lte: endDate }
            }
        });
        ...
        while (current <= endDate) {
            ...
            const monthlyData = allInteractions.filter(i => i.createdAt.getFullYear() === year && i.createdAt.getMonth() === month);
